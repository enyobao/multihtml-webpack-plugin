...
module.exports.compileTemplate = function compileTemplate(template, context, outputFilename, compilation) {
    ...
    (childCompiler.hooks ? childCompiler.hooks.compilation.tap.bind(childCompiler.hooks.compilation, 'HtmlWebpackPlugin') : childCompiler.plugin.bind(childCompiler, 'compilation'))(compilation2 => {
        if (compilation2.cache) {
            if (!compilation2.cache[compilerName]) {
                compilation2.cache[compilerName] = {};
            }
            compilation2.cache = compilation2.cache[compilerName];
        }
    });
